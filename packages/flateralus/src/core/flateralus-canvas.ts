...
export class FlateralusCanvas {
  ...
  private setupAnimation(): Sprite {
    return new this.config.animationSprite(
      this.context,
      this.config.animationSpriteConfig
    );
  }
  ...
  private startAnimation(): void {
    if (this.animationFrameId === -1) {
      // Changed to check for -1 explicitly
  ...
  private stopAnimation(): void {
    if (this.animationFrameId !== -1) {
      // Check if an animation frame is active
