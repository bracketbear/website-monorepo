...
export class RepulsionBehavior implements Behavior<RepulsionBehaviorConfig> {
  execute(
    sprite: Sprite,
    config: RepulsionBehaviorConfig,
    context: DrawContext
  ): void {
    ...
    const distanceToPointer = Math.hypot(
      pointer.position.x - sprite.position.x,
      pointer.position.y - sprite.position.y
    );
    const maxDistance = pointer.diameter * newConfig.mouseRadius;
    const angle = Math.atan2(
      sprite.position.y - pointer.position.y,
      sprite.position.x - pointer.position.x
    );
    const influence = (maxDistance - distanceToPointer) / maxDistance;

    sprite.position.x +=
      Math.cos(angle) * newConfig.repulsionStrength * influence;
    sprite.position.y +=
      Math.sin(angle) * newConfig.repulsionStrength * influence;
  }
}
