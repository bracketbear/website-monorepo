...
describe('CSS Arbitrary Value Scanner', () => {
  describe('calculateLikelihood', () => {
    it('should give high scores to px values', () => {
      ...
      const score2 = calculateLikelihood('4px_4px_0_rgba(0,0,0,0.3)', 'shadow-[4px_4px_0_rgba(0,0,0,0.3)]');
    ...
    it('should give very high scores to known CSS variables', () => {
      const score = calculateLikelihood('var(--color-primary)', 'bg-[var(--color-primary)]');
    ...
    it('should give high scores to shadow patterns with px', () => {
      const score = calculateLikelihood('8px_8px_0_#000', 'shadow-[8px_8px_0_#000]');
      expect(score).toBeGreaterThanOrEqual(85);
    });

    it('should give lower scores to custom shadows with CSS variables', () => {
      const score = calculateLikelihood('4px_4px_0_var(--color-border)', 'shadow-[4px_4px_0_var(--color-border)]');
    ...
    it('should give low scores to complex gradients', () => {
      const score = calculateLikelihood('linear-gradient(45deg, red, blue)', 'bg-[linear-gradient(45deg, red, blue)]');
  ...
  describe('getSuggestedUtilities', () => {
    ...
    it('should suggest rem alternatives for px values', () => {
      ...
      expect(suggestions).toContain('Consider using rem or em for responsive design');
  ...
  describe('getLikelihoodReason', () => {
    ...
    it('should identify known CSS variables', () => {
      const reason = getLikelihoodReason('var(--color-primary)', 'bg-[var(--color-primary)]');
      expect(reason).toContain('✅ Known CSS variable');
    });

    it('should identify shadow patterns', () => {
      const reason = getLikelihoodReason('4px_4px_0_rgba(0,0,0,0.3)', 'shadow-[4px_4px_0_rgba(0,0,0,0.3)]');
    ...
    it('should combine multiple reasons', () => {
      const reason = getLikelihoodReason('4px_4px_0_var(--color-primary)', 'shadow-[4px_4px_0_var(--color-primary)]');
      expect(reason).toContain('🚨 Uses px units');
      expect(reason).toContain('✅ Known CSS variable');
      expect(reason).toContain('🚨 Shadow with px units');
    });
  ...
  describe('extractArbitraryValues', () => {
    ...
    it('should extract multiple arbitrary values', () => {
      const classString = 'bg-[var(--color-primary)] text-[16px] shadow-[4px_4px_0_#000]';
      const values = extractArbitraryValues(classString);
  ...
  describe('Real-world scenarios', () => {
    it('should handle complex button styles', () => {
      const classString = 'inline-flex items-center px-4 py-2 text-[14px] font-medium shadow-[2px_2px_0_rgba(0,0,0,0.1)] hover:shadow-[4px_4px_0_rgba(0,0,0,0.2)]';
      const values = extractArbitraryValues(classString);
    ...
    it('should handle complex calc expressions', () => {
      const classString = 'min-h-[calc(100vh - 120px)] max-w-[calc(100% - 2rem)]';
      const values = extractArbitraryValues(classString);
