...
export default function LayeredDemoController() {
  ...
  const getSlot = (name: string) =>
    typeof window !== 'undefined' ? document.querySelector(`[slot="${name}"]`) : null;
  ...
  return (
    <>
      ...
      {typeof window !== 'undefined' && getSlot('transientUI') &&
        createPortal(
          <TooltipOverlay open={showTooltip} onClose={() => setShowTooltip(false)} />, getSlot('transientUI')
        )}
      {typeof window !== 'undefined' && getSlot('modal') &&
        createPortal(
          <ModalOverlay open={showModal} onClose={() => setShowModal(false)} />, getSlot('modal')
        )}
      {typeof window !== 'undefined' && getSlot('notifications') &&
        createPortal(
          <NotificationOverlay open={showNotification} onClose={() => setShowNotification(false)} />, getSlot('notifications')
        )}
      {typeof window !== 'undefined' && getSlot('debugHUD') &&
        createPortal(
          <DebugHUDOverlay open={showDebug} onClose={() => setShowDebug(false)} />, getSlot('debugHUD')
        )}
    </>
  );
} 